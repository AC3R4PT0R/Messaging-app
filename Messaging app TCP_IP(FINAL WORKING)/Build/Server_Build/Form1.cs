...
namespace Server_Build
{
    ...
    {
        private List<MessageData> pendingMessages = new List<MessageData>();
        ...
        private Dictionary<string, GroupChat> groupChats = new Dictionary<string, GroupChat>();
        ...
        private void HandleClient(ClientHandler clientHandler)
        {
            ...
            lock (connectedClients)
            {
                ...
                ControlInvoke(lbl_clients, () => lbl_clients.Text = $"Clients: {connectedClients.Count}"); // Update the label

                // Broadcast the updated list of connected clients after a new client connects.
            ...
            try
            {
                ...
                while (true)
                {
                    int bytesRead = streamHandler.Read(buffer, 0, buffer.Length);
                    if (bytesRead == 0)
                    {
                        // Client disconnected
                        break;
                    ...
                    if (messageParts.Length == 2)
                    {
                        ...
                        if (clients.ContainsKey(recipient))
                        {
                            ...
                            clients[recipient].Stream.Write(messageToForward, 0, messageToForward.Length);
                        }
                        else
                        {
                            // Client not connected to the server
                            // Store the message and the sender/receiver addresses
            ...
            finally
            {
                ...
                // Broadcast the updated list of connected clients after a client disconnects.
                BroadcastConnectedClients();
            }
        }
